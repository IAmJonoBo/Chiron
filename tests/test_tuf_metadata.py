...
class TestTUFMetadata:
    ...
    def test_generate_targets_metadata_hashes(
        self, tmp_repo: Path, tmp_artifacts: list[Path]
    ):
    ...
    def test_generate_targets_metadata_custom_info(
        self, tmp_repo: Path, tmp_artifacts: list[Path]
    ):
    ...
    def test_generate_targets_metadata_length(
        self, tmp_repo: Path, tmp_artifacts: list[Path]
    ):
    ...
    def test_generate_targets_metadata_with_nonexistent_file(
    ...
    def test_detect_platform_linux(self, tmp_repo: Path):
        ...
        assert (
            tuf._detect_platform("package-1.0-py3-none-manylinux_2_28_x86_64.whl")
            == "linux"
        )
    ...
    def test_detect_platform_macos(self, tmp_repo: Path):
        ...
        assert (
            tuf._detect_platform("package-1.0-py3-none-macosx_10_9_x86_64.whl")
            == "macos"
        )
...
class TestCreateTUFRepo:
    ...
    def test_create_tuf_repo_with_string_path(
        self, tmp_repo: Path, tmp_artifacts: list[Path]
    ):
    ...
    def test_create_tuf_repo_validates_metadata(
        self, tmp_repo: Path, tmp_artifacts: list[Path]
    ):
...
class TestTUFMetadataIntegration:
    ...
    def test_metadata_expiration_times(self, tmp_repo: Path):
        ...
        targets_expires = datetime.fromisoformat(
            targets["expires"].replace("Z", "+00:00")
        )
        snapshot_expires = datetime.fromisoformat(
            snapshot["expires"].replace("Z", "+00:00")
        )
        timestamp_expires = datetime.fromisoformat(
        ...
        assert (snapshot_expires - now).days * 24 > (
