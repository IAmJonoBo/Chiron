...
class DependencyPolicy:
    ...
    def from_toml(cls, config_path: Path) -> DependencyPolicy:
        ...
        policy = cls(
            ...
            default_upgrade_cadence_days=policy_data.get(
                "default_upgrade_cadence_days"
            ),
...
class PolicyEngine:
        ...
        if pkg_policy.version_ceiling:
            try:
                ...
                if ver > ceiling:
                    return (
                        False,
                        f"Version exceeds ceiling: {pkg_policy.version_ceiling}",
                    )
            except Exception:
                pass
    ...
    def check_upgrade_allowed(
        ...
        pkg_policy = self.policy.allowlist.get(package_name)
        cadence = (
            pkg_policy.upgrade_cadence_days
            if pkg_policy
            else self.policy.default_upgrade_cadence_days
        )
