...
class ChironWizard:
    ...
    def build_wheelhouse_wizard(self) -> dict[str, Any]:
        ...
        console.print(
            Panel.fit(
                "[bold cyan]Wheelhouse Build Wizard[/bold cyan]",
                border_style="cyan"
            )
        )
        console.print()
        ...
        with_sbom = Confirm.ask(
            "Generate SBOM?",
            default=True
        )
        
        with_signatures = Confirm.ask(
            "Sign artifacts with Sigstore?",
            default=True
        )
        
        with_vuln_scan = Confirm.ask(
    ...
    def verify_wizard(self) -> dict[str, Any]:
        ...
        verify_signatures = Confirm.ask(
            "Verify signatures?",
            default=True
        )
        
        verify_sbom = Confirm.ask(
            "Verify SBOM?",
            default=True
        )
        
        verify_provenance = Confirm.ask(
            "Verify provenance?",
            default=True
        )
        
        verify_hashes = Confirm.ask(
            "Verify checksums?",
            default=True
        )
    ...
    def _display_summary(self, config: dict[str, Any]) -> None:
        ...
        table.add_row("Python Versions", ", ".join(config["wheelhouse"]["python_versions"]))
