...
class FeatureFlags:
    ...
    DEFAULT_FLAGS = {
        ...
        ) if OPENFEATURE_AVAILABLE else None,
    ...
    def client(self):
        ...
        if not OPENFEATURE_AVAILABLE:
            raise ImportError("OpenFeature not available. Install with: pip install openfeature-sdk")
        return api.get_client()
    
    def get_boolean(self, flag_key: str, default: bool = False, context: dict[str, Any] | None = None) -> bool:
        ...
        if not OPENFEATURE_AVAILABLE or not self._initialized:
            ...
            return os.getenv(env_key, str(default)).lower() in ("true", "1", "yes", "on")
    ...
    def get_string(self, flag_key: str, default: str = "", context: dict[str, Any] | None = None) -> str:
