...
and git churn data. It complements the built-in `hephaestus tools refactor hotspots`
...
Example:
    python hotspots_py.py src/hephaestus --output hotspots.csv
"""
...
def parse_args():
    ...
    parser.add_argument(
        "--churn-file",
        default="hephaestus-toolkit/refactoring/output/churn.txt",
        help="Path to churn file from git_churn.sh",
    )
    parser.add_argument(
        "--output",
        default="hephaestus-toolkit/refactoring/output/hotspots.csv",
...
def get_complexity(src_dir: str) -> dict[str, int]:
    ...
    try:
        ...
        print(
            "Warning: radon not found. Install with: uv pip install radon",
            file=sys.stderr,
        )
