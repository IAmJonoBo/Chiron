...
def test_tools_quality_suite_renders_progress(monkeypatch: MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "hephaestus.cli.load_quality_configuration",
        lambda: QualityConfiguration(gates={}, profiles={}),
    )
    monkeypatch.setattr(
        "hephaestus.cli.available_quality_profiles",
        lambda config: {"demo": tuple(gate.name for gate in plan.gates)},
    )
    monkeypatch.setattr(
        "hephaestus.cli.build_quality_suite_plan",
    ...
    monkeypatch.setattr(
        "hephaestus.cli.execute_quality_suite", fake_execute_quality_suite
    )
...
def test_tools_docs_sync_diataxis(monkeypatch: MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr("hephaestus.cli.sync_diataxis_documentation", fake_sync)
...
def test_tools_docs_sync_diataxis_with_discovery(
    ...
    monkeypatch.setattr(
        "hephaestus.cli.discover_diataxis_entries",
        lambda docs: fake_entries if docs == docs_dir else {},
    )
    monkeypatch.setattr(
        "hephaestus.cli.dump_diataxis_entries", lambda path, entries: path
    )
...
def test_tools_refactor_analyze_outputs_summary(monkeypatch: MonkeyPatch) -> None:
    opportunity = RefactorOpportunity(
        path=Path("src/hephaestus/example.py"),
        line=42,
        symbol="example",
    ...
    monkeypatch.setattr(
        "hephaestus.cli.analyze_refactor_opportunities",
...
def test_tools_refactor_analyze_json_output_is_sorted(monkeypatch: MonkeyPatch) -> None:
    opportunity_high = RefactorOpportunity(
        path=Path("src/hephaestus/high.py"),
        line=5,
        symbol="complex",
    ...
    opportunity_low = RefactorOpportunity(
        path=Path("src/hephaestus/low.py"),
        line=3,
        symbol="helper",
        kind="function_length",
        severity="info",
...
def test_tools_refactor_hotspots_outputs_summary(monkeypatch: MonkeyPatch) -> None:
    entry1 = HotspotEntry(
        path=Path("src/hephaestus/complex.py"),
        complexity_score=500,
        churn_count=10,
        hotspot_score=5000,
    )
    entry2 = HotspotEntry(
        path=Path("src/hephaestus/simple.py"),
        complexity_score=50,
        churn_count=5,
        hotspot_score=250,
    )
    ...
    monkeypatch.setattr(
        "hephaestus.cli.analyze_hotspots",
...
def test_tools_refactor_hotspots_json_output(monkeypatch: MonkeyPatch) -> None:
    entry = HotspotEntry(
        path=Path("src/hephaestus/test.py"),
    ...
    assert payload["entries"][0]["path"] == "src/hephaestus/test.py"
